...
export const Pricing = () => {
  return (
    <section id="planes" className="w-full py-8 px-6 md:px-10 max-w-7xl mx-auto">
        ...
        {PLANS.map((plan) => (
          <div
            ...
            className={`relative p-8 rounded-2xl transition-all duration-500 flex flex-col h-full animate-fade-up opacity-0
              ${plan.recommended
                ? "bg-white border-3 border-secondary shadow-lg scale-105 z-10 md:-translate-y-4 md:animate-float"
                : "bg-secondary-light/20 border-2 border-transparent hover:border-gray-200"
              }`}
          >
            {plan.recommended && (
              <div className="absolute -top-5 left-1/2 -translate-x-1/2 bg-secondary text-white text-xs font-black tracking-widest py-2 px-6 rounded-full z-20">
                MÁS POPULAR
              </div>
            )}
            ...
            <div className="mb-8 relative z-10">
              <h3 className="text-2xl font-heading font-black text-primary mb-2">{plan.name}</h3>
              <div className="flex items-baseline gap-1">
                <span className="text-sm font-bold text-gray-400 uppercase">S/</span>
                <span className="text-5xl font-heading font-black text-primary">{plan.price}</span>
              ...
              {plan.features.map((feature) => (
                <div key={feature} className="flex items-start gap-3 group">
                  <CheckCircleIcon
                    size={22}
                    weight="fill"
                    className={plan.recommended ? "text-secondary" : "text-secondary-light"}
                  />
            ...
            <button
              className={`btn-base w-full transition-all active:scale-95 relative z-10
                ${plan.recommended
                  ? "bg-secondary text-white hover:bg-secondary hover:brightness-105 transition-all"
                  : "bg-white border-2 border-secondary/20 text-secondary hover:border-secondary"}`}
            >
              {plan.cta}
            </button>
          </div>
